...
def send_discovery():
    ...
    try:
        ...
        logger.info(f"🛰️ Sending discovery to {settings.WORKER_UDP_BROADCAST_IP}:{settings.COORDINATOR_UDP_PORT}")
        sock.sendto(message.encode("utf-8"), (settings.WORKER_UDP_BROADCAST_IP, settings.COORDINATOR_UDP_PORT))
        sock.close()
...
def run_llama_server():
    ...
    try:
        ...
        if os.path.exists(settings.WORKERS_FILE):
            with open(settings.WORKERS_FILE, "r") as f:
                worker_list = [line.strip() for line in f if line.strip()]
                # worker format: IP:RANK
                # Sort by rank to determine ring order
                worker_list.sort(key=lambda x: int(x.split(":")[1]))
                
                # Master is rank 0
                master_ip = worker_list[0].split(":")[0]
                
                # Next node (ring structure)
                my_rank = settings.WORKER_RANK
                for i, w in enumerate(worker_list):
                    if int(w.split(":")[1]) == my_rank:
                        next_worker = worker_list[(i + 1) % len(worker_list)]
                        next_ip = next_worker.split(":")[0]
                        break
    except Exception as e:
        logger.warning(f"⚠️ Could not fully determine swarm topology: {e}")
